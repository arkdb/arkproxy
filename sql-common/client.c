				...
				const char *filename,const char *group)
{
  ...
  my_load_defaults(filename, groups, &argc, &argv, NULL, NULL);
...
typedef struct {
  ...
  /* for proxy*/
  int connection_type ;
  const uint8 *hash_stage2;/* 从数据库中读取过来的密码，被处理为bin格式*/
} MCPVIO_EXT;
...
int proxy_auth_backend_server(
    MYSQL *mysql, 
    char *data, 
    uint data_len,
    const char *data_plugin, 
    const char *db,
    const uint8 *hash_stage1
)
{
  const char    *auth_plugin_name;
  auth_plugin_t *auth_plugin;
  ...
  mpvio.db= db;
  mpvio.plugin= auth_plugin;
  mpvio.connection_type = PROXY_CONNECT_PROXY;
  mpvio.hash_stage2 = hash_stage1;
...
MYSQL *proxy_mysql_client_methods(MYSQL *mysql) {
  mysql->methods= &client_methods;
  mysql->client_flag=0;			/* For handshake */
}

MYSQL * 
proxy_connect_backend_server(
    MYSQL* mysql,
    const char *host, 
    ...
    uint source_port,
    ulong client_flag, 
    const uint8 *hash_stage1 /* 解密出来的hash_stage1 */
)
{
  ...
  */
  ...
  {
      char proxy_buf[256];
      sprintf(proxy_buf, "PROXY TCP4 %s %s %d %d\r\n", source_host, host, source_port, port);
      vio_write(net->vio, proxy_buf, strlen(proxy_buf));
  }
  /* Get version info */
  ...
  */
  ...
  /* 这里的scramble_data，就是原始的scramble数据，但验证完成之后，就会被修改掉 */
  if (proxy_auth_backend_server(mysql, scramble_data, scramble_data_len,
                      scramble_plugin, db, hash_stage1))
    goto error;
		       ...
		       uint port, const char *unix_socket,ulong client_flag)
{
  ...
  */
  ...
  {
      char proxy_buf[256];
      sprintf(proxy_buf, "PROXY TCP4 %s %s 0 %d\r\n", proxy_server_address, host, port);
      vio_write(net->vio, proxy_buf, strlen(proxy_buf));
  }
...
static int native_password_auth_client(MYSQL_PLUGIN_VIO *vio, MYSQL *mysql)
{
    ...
    mpvio = (MCPVIO_EXT *)vio;
    if (((MCPVIO_EXT *)vio)->connection_type == PROXY_CONNECT_PROXY)
    {
        ...
        scramble_replace_scramble(scrambled, mysql->scramble, 
                (const char*)mpvio->hash_stage2);
