...
%type <NONE>
	...
	keep_gcc_happy
        ...
        explainable_command
        /* for proxy */
        config_proxy proxy_router_hint config_user config_user_option
...
query:
        ...
        | proxy_router_hint verb_clause 
          ...
          ';'
          opt_end_of_input
        | proxy_router_hint verb_clause END_OF_INPUT
          {
...
proxy_router_hint:
        /* empty */
        {
            thd->lex->route_type = 0;
        }
        | ROUTE_SYM TO_SYM route_type
        {
            thd->lex->route_type = $3;
        }
        | NO_SYM HASH_SYM
        {
            thd->lex->route_type = ROUTER_TYPE_NOHASH;
        }
        | BACKEND_SYM
        {
            thd->lex->route_type = ROUTER_TYPE_CONNECTION_BACKEND;
        }
        ;
...
config_user:
        config_user_option
        | config_user ',' config_user_option
        ;

config_user_option:
        MAX_USER_CONNECTIONS_SYM '='  ulonglong_num
        {
            if (!thd->config_ele)
                thd->config_ele = (config_element_t*)my_malloc(
                sizeof(config_element_t), MY_ZEROFILL);
            thd->config_ele->max_connections = $3;
        }
        | DATABASES '=' ident_or_text
        {
            if (!thd->config_ele)
                thd->config_ele = (config_element_t*)my_malloc(
                sizeof(config_element_t), MY_ZEROFILL);
            strcpy(thd->config_ele->dbname, $3.str);
        };
...
server_attribute:
        HOST_SYM '=' TEXT_STRING_sys
        {
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
          strcpy(thd->config_ele->hostname, $3.str);
        }
        | PORT_SYM '=' ulonglong_num
        {
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
          thd->config_ele->port = $3;
        }
        | MAX_LAG '=' ulonglong_num
        {
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
        ...
        | WEIGHT_SYM '=' ulonglong_num
        {
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
          thd->config_ele->weight = $3;
        }
        ;

config_proxy:
        CONFIG_SYM ADD route_type SERVER_SYM ident server_attributes
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_ADD_SERVER;
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
        ...
        | CONFIG_SYM DELETE_SYM SERVER_SYM ident
        {
            LEX *lex= thd->lex;
            Lex->sql_command= SQLCOM_CONFIG;
            Lex->sub_command= CFG_DELETE_SERVER;
            if (!thd->config_ele)
                thd->config_ele = (config_element_t*)my_malloc(
                sizeof(config_element_t), MY_ZEROFILL);
            strcpy(thd->config_ele->server_name, $4.str);
        }
        | CONFIG_SYM WRITE_SYM OUTFILE 
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_WRITE;
          Lex->ident.length = 0;
        }
        | CONFIG_SYM WRITE_SYM OUTFILE TEXT_STRING_sys
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_WRITE;
        ...
        {
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_RELOAD;
        }
        | CONFIG_SYM SET SERVER_SYM ident ONLINE_SYM
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_ONLINE_SERVER;
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
          strcpy(thd->config_ele->server_name, $4.str);
          thd->config_ele->sub_command = CFG_CMD_ONLINE_SERVER;
        }
        | CONFIG_SYM SET SERVER_SYM ident OFFLINE_SYM
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_OFFLINE_SERVER;
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
          strcpy(thd->config_ele->server_name, $4.str);
          thd->config_ele->sub_command = CFG_CMD_OFFLINE_SERVER;
        }
        | CONFIG_SYM SET USER_SYM ident_or_text '@' ident_or_text config_user
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_USER_SET;
          strcpy(thd->config_ele->username, $4.str);
          strcpy(thd->config_ele->hostname, $6.str);
        }
        | CONFIG_SYM DELETE_SYM ulonglong_num
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_DELETE_CONFIG;
          lex->table_count = $3;
        }
        | CONFIG_SYM DELETE_SYM ALL
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_DELETE_CONFIG;
        ...
        | CONFIG_SYM RESET_SYM
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_DELETE_CONFIG;
        }
        | CONFIG_SYM FLUSH_SYM
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_CMD_FLUSH;
        }
        | CONFIG_SYM HELP_SYM
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CONFIG_HELP;
        }
        | CONFIG_SYM TRACE_SYM CACHE_SYM CLOSE_SYM
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_TRACE_CLOSE;
        }
        | CONFIG_SYM TRACE_SYM SQL_SYM CACHE_SYM CLOSE_SYM
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_TRACE_SQL_CLOSE;
        }
        | CONFIG_SYM TRACE_SYM CACHE_SYM CREATE
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_TRACE_CREATE;
        }
        | CONFIG_SYM TRACE_SYM SQL_SYM CACHE_SYM CREATE
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_TRACE_SQL_CREATE;
        }
        | CONFIG_SYM ADD RULE_SYM rule_options
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_RULE_ADD;
        }
        | CONFIG_SYM UPDATE_SYM RULE_SYM rule_options
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_RULE_UPDATE;
        }
        | CONFIG_SYM DELETE_SYM RULE_SYM rule_options
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_RULE_DELETE;
        }
        | CONFIG_SYM DELETE_SYM RULE_SYM ALL
        {
          LEX *lex= thd->lex;
          Lex->sql_command= SQLCOM_CONFIG;
          Lex->sub_command= CFG_RULE_DELETE_ALL;
          if (!thd->config_ele)
              thd->config_ele = (config_element_t*)my_malloc(
              sizeof(config_element_t), MY_ZEROFILL);
          thd->config_ele->sub_command = CFG_RULE_DELETE_ALL;
        }
        ;
...
rule_option:
        ident '=' NULL_SYM
        {
            create_rule_if_null(thd, $1.str, LLONG_MAX, NULL);
        }
        | ident '=' TEXT_STRING_sys
        {
            create_rule_if_null(thd, $1.str, LLONG_MAX, $3.str);
        }
        | ident '=' ulonglong_num
        {
            create_rule_if_null(thd, $1.str, $3, NULL);
        }
        ;
...
predicate:
          ...
          {
            Item *item;
            item = new (thd->mem_root) Item_in_subselect(thd, $1, $4);
...
simple_expr:
        ...
        | simple_ident JSON_SEPARATOR_SYM TEXT_STRING_literal
          ...
          /*
            Item_string *path=
              new (thd->mem_root) Item_string(thd, $3.str, $3.length,
                                                      thd->charset());
            $$= new (thd->mem_root)Item_func_json_extract(thd, path);
            */
            $$ = $1;
          }
         | simple_ident JSON_UNQUOTED_SEPARATOR_SYM TEXT_STRING_literal
          ...
          /*
            Item_string *path=
              new (thd->mem_root)Item_string(thd, $3.str, $3.length,
                                                      thd->charset());
            Item *extr= new (thd->mem_root)Item_func_json_extract(thd, path);
            $$= new (thd->mem_root)Item_func_json_unquote(thd, extr);
            */
            $$ = $1;
          }
        ;
...
variable:
          '@'
          {
            /* if the sql use the local var, then route to write node */
            Lex->route_type = ROUTER_TYPE_RW;
...
join_table:
          ...
          */
          ...
          { MYSQL_YYABORT_UNLESS($1 && ($$=$3)); $3->straight=$2; $3->natural_join_type=1;}
...
proxy_show_param:
           CONNECTIONS_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_CONN;
           }
           | THREADPOOL_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_THREADPOOL;
           }
           | BACKEND_SYM CONNECTIONS_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_BACKEND_CONN;
           }
           | PROXY_SYM STATUS_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_PROXY_STATUS;
           }
           | PROXY_SYM CONNECTION_SYM STATUS_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_PROXY_CONN_STATUS;
           }
           | USER_SYM CONFIG_SYM LIST_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= CFG_CMD_SHOW_USER_CONFIG_LIST;
           }
           | CONFIG_SYM CACHE_SYM 
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= CFG_CMD_SHOW_CACHE_LIST;
           }
           | BACKEND_SYM SERVERS_SYM 
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_BACKEND_SERVERS;
           }
           | BACKEND_SYM ROUTES_SYM 
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_BACKEND_ROUTES;
           }
           | TRACE_SYM STATUS_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_TRACE_STATUS;
           }
           | TRACE_SYM FULL STATUS_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SHELL_CMD_FULL_TRACE_STATUS;
           }
           | RULE_SYM STATUS_SYM
           {
             LEX *lex= Lex;
             lex->sql_command= SQLCOM_CONFIG;
             lex->sub_command= SQLCOM_SHOW_RULE_STATUS;
           }
           ;
...
wild_and_where:
        ...
        | LOCAL_SYM { Lex->type= TYPE_ENUM_PROXY; }
        ;
...
kill_option:
          /* empty */    { $$= (int) KILL_CONNECTION; Lex->type= 0;}
        | CONNECTION_SYM { $$= (int) KILL_CONNECTION; Lex->type= 0;}
        | QUERY_SYM      { $$= (int) KILL_QUERY; Lex->type= 1;}
        | QUERY_SYM ID_SYM
          {
            $$= (int) KILL_QUERY;Lex->type= 1;
...
option_value_following_option_type:
          internal_variable_name equal set_expr_or_default
          {
            ...
            if ($1.var != NULL || thd->connection_type != PROXY_CONNECT_PROXY)
            {
        ...
        | charset old_or_new_charset_name_or_default
          {
            ...
            var= (new (thd->mem_root)
                  set_var_collation_client(flags,
                                            cs2,
                                            thd->variables.collation_database,
                                            cs2));
            if (var == NULL)
