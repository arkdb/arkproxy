...
#include "sql_rules.h"            // sql_digest_state
...
#define ROUTER_TYPE_RO                  1
#define ROUTER_TYPE_RW                  2
#define ROUTER_TYPE_NOHASH              3
#define ROUTER_TYPE_CONNECTION_BACKEND  4

#define LISTEN_TYPE_PROXY 1
#define LISTEN_TYPE_SHELL 2

#define SERVER_STATUS_ONLINE 1
#define SERVER_STATUS_OFFLINE 2
...
typedef struct backend_env_struct backend_env_t;
struct backend_env_struct{
    enum enum_sql_command type;
    str_t str;
    str_t name;
    uint env_version;

    LIST_NODE_T(backend_env_struct) link;
};

typedef struct proxy_router_struct proxy_router_t;
struct proxy_router_struct
{
    char router_name[MAX_HOSTNAME];
    int router_type;
    char* servers;
...
typedef struct proxy_white_ip_struct proxy_white_ip_t;
struct proxy_white_ip_struct
{
    char ip[16];
    LIST_NODE_T(proxy_white_ip_struct) link;
};

typedef struct proxy_server_struct proxy_server_t;
struct proxy_server_struct
{
...
typedef struct proxy_listens_struct proxy_listens_t;
struct proxy_listens_struct
{
    char* comments;
    /*shell*/
    char* shell_username;
    ...
    int listen_type;
    int listen_port;
    LIST_NODE_T(proxy_listens_t) link;
};
...
struct address_struct{
    char hostname[256];
    ulong port;
    LIST_NODE_T(address_t) link;
};

typedef struct proxy_config_struct proxy_config_t;
struct proxy_config_struct{
    uint config_version; /* if server add or online, then set it to true to reconnect */
    uint rules_version; /* if server add or online, then set it to true to reconnect */
    /* pointer to server base info */
    LIST_BASE_NODE_T(proxy_server_t) rw_server_lst;
    LIST_BASE_NODE_T(proxy_server_t) ro_server_lst;

    LIST_BASE_NODE_T(proxy_listens_t) listen_lst;
    LIST_BASE_NODE_T(proxy_router_t) router_lst;
    LIST_BASE_NODE_T(proxy_servers_t) server_lst;
    LIST_BASE_NODE_T(proxy_white_ip_t) white_ip_lst;
    LIST_BASE_NODE_T(address_t) incoming_lst;
    LIST_BASE_NODE_T(QP_rule_t) rules_lst;
...
struct config_element_struct{
    uint sequence;
    enum_sub_command sub_command;
    char  server_name[USERNAME_CHAR_LENGTH + 1];
    char  username[USERNAME_CHAR_LENGTH + 1];
    char  hostname[USERNAME_CHAR_LENGTH + 1];
    char  dbname[USERNAME_CHAR_LENGTH + 1];
    str_t config_command;
    ulong max_connections;
    ulong port;
    ulong weight;
    ulong type;
    QP_rule_t* rule;
    LIST_NODE_T(config_element_t) link;
};

typedef struct config_cache_struct config_cache_t;
struct config_cache_struct{
    LIST_BASE_NODE_T(config_element_t) config_lst;
    LIST_BASE_NODE_T(config_element_t) sync_lst;
};

#if __CONSISTEND_READ__

typedef struct proxy_consistend_read_struct proxy_consistend_read_t;
struct proxy_consistend_read_struct{
    int instance;
    char  dbname[128 + 1];
    /* the space is not enough when set too much dbs*/
    char  tablename[128 + 1];
    longlong timestamp;
    LIST_NODE_T(proxy_consistend_read_t) link;
};

typedef struct consistend_read_cache_struct consistend_cache_t;
struct consistend_read_cache_struct{
    mysql_mutex_t consistend_lock;
    LIST_BASE_NODE_T(proxy_consistend_read_t) consistend_read_lst;
};
#endif 

typedef struct proxy_user_struct proxy_user_t;
struct proxy_user_struct{
...
struct backend_conn_struct {
    void* m_mysql;
    char  user[USERNAME_CHAR_LENGTH + 1];
    char  passwd[MAX_PASSWORD_LENGTH + 1];
    char  host[HOSTNAME_LENGTH + 1];
    char  db[NAME_CHAR_LEN];
    char  md5_hash[USERNAME_CHAR_LENGTH + 1];
    uint port;
    ulong wait_timeout;
    ulong slave_lag;

#if __CONSISTEND_READ__
    consistend_cache_t consistend_cache2;
    consistend_cache_t consistend_cache1;
...
typedef struct stmtid_conn_struct stmtid_conn_t;
struct stmtid_conn_struct{
    unsigned long	 stmt_id;	       /* Id for prepared statement */
    /* in case of multi write, the prepare stmt id is generated by 
     * multi server, so it is prossible same with different backend 
     * server, so the client stmt need to menaget by proxy */
    unsigned long	 client_stmt_id;	       /* Id for prepared statement */
    backend_conn_t* stmt_conn;
    
    LIST_NODE_T(stmtid_conn_t) link;
};

typedef struct stmtid_cache_struct stmtid_cache_t;
struct stmtid_cache_struct{
    LIST_BASE_NODE_T(stmtid_conn_t) stmtid_lst;
};
typedef struct ok_info_struct ok_info_t;
struct ok_info_struct{
    uint affected_rows;
    uint insert_id;
    uint server_status;
    uint warning_count;
...
typedef struct net_packet_struct net_packet_t;
struct net_packet_struct{
    /* 包括NET_STATE_NULL, NET_STATE_FULL, NET_STATE_END, NET_STATE_ERROR */
    volatile int net_state; 
    volatile int sub_net_state; 
    enum enum_server_command command;
    enum enum_sql_command sub_command;
    str_t buffer;
    /* for error/ok packet*/
    int error_code;
    ...
    ok_info_struct ok_info;
    uint field_count, param_count, warning_count;
    unsigned long	 stmt_id;	       /* Id for prepared statement */
};

typedef struct net_queue_struct net_queue_t;
struct net_queue_struct{
    net_packet_t* packets;    
...
typedef struct table_info_struct table_info_t;
struct table_info_struct
{
    ...
    int     new_cache;
    /*
     * 来表示对当前表操作是不是ddl与dml共存,
     * 如果共存则报错，因为如果备份的话会有问题
     * */
    int     dmlddl;
    int     new_column_cache;
    int     alter_count;//记录这个表被多少个alter语句修改过
...
struct format_cache_node_struct
{
    ...
    ulonglong          min_time;
    ulonglong          max_time;
    ulonglong          affect_rows;
    rt_lst_t           rt_lst;
    mysql_mutex_t trace_lock;
...
proxy_servers_t* get_master_instance();
int connection_add_master(THD* thd, void* mysql, char* user, char* host, uint port);
backend_conn_t* 
add_write_connection(THD* thd, proxy_servers_t* server, char* user, char* md5_hash, char* host, uint port);
backend_conn_t* 
add_read_connection(THD* thd, proxy_servers_t* server, char* user, char* md5_hash, char* host, uint port);
ulong mysql_fetch_wait_timeout(void* mysql);
...
backend_conn_t* get_cluster_connection(backend_conn_t* conn, uint count,
                                       char* user, char* password, uint index);
bool get_cluster_auth_passwd(THD* thd, char* user, char* host, char* ip, char* password);
...
void str_reset_last_len(str_t* str);
void str_deinit(str_t* str);
char* str_get(str_t* str);
int str_get_len(str_t* str);
int str_get_alloc_len(str_t* str);
int str_get_last_len(str_t* str);
int str_set_last_len(str_t* str);
void proxy_free_config_cache(THD* thd);
...
bool write_eof_packet(THD *, NET *, uint, uint);
#endif
...
typedef struct trace_queue_struct trace_queue_t;
struct trace_queue_struct{
    format_cache_node_t* trace_array;
    int queue_size;
    /* state include TRACE_STATE_BUSY, TRACE_STATE_EMPTY */
    volatile int trace_state;
    volatile int thd_state;
    volatile int enqueue_index;
    volatile int dequeue_index;
    volatile int count;
    mysql_mutex_t trace_lock;
    ...
    LIST_NODE_T(trace_queue_t)         link;
    THD* thd;
};

typedef struct trace_flush_struct trace_flush_t;
struct trace_flush_struct{
    mysql_mutex_t flush_lock;
    mysql_cond_t flush_cond;
    /* 两个HASH表为了减少冲突，交换使用 */
    HASH* en_trace_hash;
    HASH* de_trace_hash;
    /* 用来标识两个HASH表的状态，为false说明没有在使用，只要在使用，另一方就等待或者忽略 */
    volatile int detrace;
    volatile int entrace;
    ...
    LIST_BASE_NODE_T(format_cache_node_t) en_free_list;
    LIST_BASE_NODE_T(format_cache_node_t) de_free_list;
};

typedef struct trace_sql_bucket_struct trace_sql_bucket_t;
struct trace_sql_bucket_struct{
    mysql_mutex_t bucket_lock;
    mysql_cond_t bucket_cond;
    format_cache_node_t* sql_bucket;
    volatile int bucket_enqueue_index;
    volatile int bucket_dequeue_index;
    int bucket_size;
    int running;
...
typedef struct timeout_struct timedout_t;
struct timeout_struct{
    /* first trace cache for client thd */
    proxy_servers_t* server;
    int64_t start_time;
    int timeout;
    my_thread_id  thread_id;
    LIST_NODE_T(timedout_t)         link;
};

typedef struct timeout_cache_struct timeout_cache_t;
struct timeout_cache_struct{
    /* first trace cache for client thd */
    timedout_t* timeout_queue;
    LIST_BASE_NODE_T(timedout_t) free_list;
    LIST_BASE_NODE_T(timedout_t) full_list;
    int timeout_cache_size;
    mysql_mutex_t timeout_lock;
};

struct query_rules_struct
{
    //query rules, digest 
    uint64_t digest;
    char *digest_text;
    ...
    uint rule_version;
    std::vector<QP_rule_t *> * _thr_SQP_rules;
    timedout_t* timeout_thd;
    //std::vector<QP_rule_t *> rules;
};

typedef struct query_rules_struct query_rules_t;

typedef struct trace_cache_struct trace_cache_t;
struct trace_cache_struct{
    /* first trace cache for client thd */
    trace_queue_t* trace_queue;
    LIST_BASE_NODE_T(trace_queue_t) free_list;
    LIST_BASE_NODE_T(trace_queue_t) full_list;
    int trace_cache_size;
    /* another trace cache for flush thread */
    trace_flush_t* trace_flush;
    int trace_flush_size;
    
    trace_sql_bucket_t* trace_sql_bucket;
    int trace_sql_bucket_size;
    my_bool local_proxy_sql_trace;
    my_bool local_proxy_digest_trace;
    
    //rd_kafka_topic_t* trace_sql_rkt;
    //rd_kafka_topic_t* trace_digest_rkt;
    void* trace_sql_rkt;
    void* trace_sql_rk;
    void* trace_digest_rkt;
    void* trace_digest_rk;

    int trace_version;
    int trace_storage_type;

    /* mutex */
    mysql_mutex_t trace_lock;
    mysql_cond_t format_thread_cond;
    mysql_mutex_t format_thread_lock;
    int trace_digest_thread_start_count;
    int trace_digest_thread_stop_count;
};
...
public:
    ...
    backend_conn_t* last_trx_conn;
    backend_conn_t* last_temp_conn;
    backend_conn_t* write_conn[MAX_CLUSTER_NODES];
    int write_conn_count;
    backend_conn_t* read_conn[MAX_CLUSTER_NODES];
    int read_conn_count;
    int first_request;
    proxy_servers_t* first_request_best_server;
#if __CONSISTEND_READ__
    int proxy_enable_consistend_read;
    ulonglong last_update_consistend_cache_time; 
    ulonglong consistend_execute_time; 
#endif
    // LIST_BASE_NODE_T(backend_conn_t) read_conn_list;
    net_queue_t queue_in;
    net_queue_t queue_out;
    volatile int64 queue_packet_buf_size;
    volatile int64 queue_in_packet_total_len;
    volatile int64 queue_out_packet_total_len;
    mysql_cond_t        executer_cond;
    mysql_mutex_t       executer_lock;
    mysql_cond_t        read_cond;
    mysql_mutex_t       read_lock;
    volatile int running;
    volatile int thread_running;
    volatile int reconfig;
    volatile int cond_wait_count;
    volatile int cond_signal_count;

    ulong thd_proxy_recv_queue_size;
    ulong thd_proxy_send_queue_size;
    query_rules_t* query_rule;
    QP_rule_t* config_rule;
    int trace_digest_on;

    LIST_BASE_NODE_T(backend_env_t) env_list;
    uint env_version;
    uint trace_version;
    uint stmt_flags;
    ulonglong last_update_slave_time; 
    config_cache_t* config_cache;
    stmtid_cache_t* stmtid_cache;
    unsigned long	 stmt_id;	       /* Id for execute/fetch/close ... statement */
    uint config_sequence;
    uint config_version; /* if server add or online, then set it to true to reconnect */
    config_element_t* config_ele;
    trace_queue_t* trace_queue;
};
...
void proxy_sql_kill(THD *thd, longlong id, killed_state state, killed_type type);
