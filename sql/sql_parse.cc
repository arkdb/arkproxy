...
bool do_command(THD *thd)
{
  ...
  */
  ...
  /* here, if the client have already closed, then the packet_length is 
   * packet_error, then in function end_connection, it will report the 
   * message: [Warning] Aborted connection 5685 to db: 'sbtest' user: 
   * 'username' host: '10.88.73.86' (Got an error reading communication packets)*/
                      ...
                      bool is_next_command)
{
  ...
  thd->trace_digest_on = proxy_digest_trace;
...
//#endif
  ...
  */
  ...
  /* retry get the trace queue */
  if ((proxy_digest_cache_on() && !thd->trace_queue) || 
      (thd->trace_queue && (int)thd->trace_version < proxy_trace_version()))
      proxy_assign_thd_trace(thd);
  ...
  case COM_REGISTER_SLAVE:
  {
    ...
    thd->set_query((char*)packet, packet_length, system_charset_info);
    proxy_dispatch_query(thd);
    break;
  }
  ...
  {
    proxy_change_user(thd, net, packet, packet_length);
    break;
  }
  ...
  {
      thd->set_query((char*)packet, packet_length, system_charset_info);
      //thd->stmt_id= uint4korr(packet);
      proxy_dispatch_query(thd);
      break;
  }
  case COM_QUERY:
  {
      proxy_execute_query_command(command, thd, packet, 
          packet_length, is_com_multi, is_next_command);
      break;
  }
  case COM_FIELD_LIST:				// This isn't actually needed
  {
      ...
      if (thd->copy_with_error(system_charset_info, &tmp,
          thd->charset(), packet, packet_length))
          break;
      thd->set_query(tmp.str, tmp.length, system_charset_info);
      proxy_dispatch_query(thd);
      break;
  }
  case COM_QUIT:
  ...
  {
      general_log_print(thd, command, NullS);
      status_var_increment(thd->status_var.com_stat[command]);
      thd->set_query((char*)packet, packet_length, system_charset_info);
      proxy_dispatch_query(thd);
      break;
  }
