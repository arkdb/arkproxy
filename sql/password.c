...
void
scramble_replace_scramble(
    char *to, 
    const char *new_scramble, 
    const char *hash_stage1
)
{
    /* uint8 hash_stage1[SHA1_HASH_SIZE]; */
    uint8 hash_stage2[SHA1_HASH_SIZE];
    uint8 newbuf[SHA1_HASH_SIZE];

    /* create key to encrypt scramble */
    /* compute_sha1_hash_multi(buf, origin_scramble, SCRAMBLE_LENGTH, */
    /*     hash_stage_2_base, SHA1_HASH_SIZE); */
    /* generate the hash_stage1 */
    /* my_crypt((char *) hash_stage1, (const uchar *)buf, 
     * (const uchar *)scramble_arg, SCRAMBLE_LENGTH); */
  
    /* generate the hash_stage2*/
    compute_sha1_hash(hash_stage2, (const char *) hash_stage1, SHA1_HASH_SIZE);
    
    /* Use the new scramble to crypt */
    compute_sha1_hash_multi((uint8 *) newbuf, new_scramble, SCRAMBLE_LENGTH,
        (const char *) hash_stage2, SHA1_HASH_SIZE);
  
    my_crypt(to, (const uchar *) newbuf, (const uchar *)hash_stage1, SCRAMBLE_LENGTH);
}
...
my_bool
caculate_first_stage_hash1(const uchar *scramble_arg, const char *message,
               const uint8 *hash_stage1, const uint8 *hash_stage2)
{
    uint8 buf[SHA1_HASH_SIZE];
    compute_sha1_hash_multi(buf, (const uchar*)message, SCRAMBLE_LENGTH,
                  (const uint8 *)hash_stage2, SHA1_HASH_SIZE);
    my_crypt((char *) hash_stage1, (const uchar *)buf, 
              (const uchar *)scramble_arg, SCRAMBLE_LENGTH);
}
