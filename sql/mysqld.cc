...
static char compiled_default_collation_name[]= "utf8mb4_unicode_ci";
...
my_bool opt_proxy_reserve_client_address= 0;
char *opt_slave_skip_errors;

char *backend_user;
char *proxy_namespace;
char *backend_passwd;
char* proxy_server_address;
char* proxy_incoming_addresses;
char* proxy_config_host;
ulong proxy_config_port;
char *proxy_config_user;
char *proxy_config_passwd;
ulong proxy_trace_storage;
char* proxy_trace_storage_host;
ulong proxy_trace_storage_port;
char *proxy_trace_storage_user;
char *proxy_trace_storage_passwd;
ulong proxy_send_queue_size;
ulong proxy_trace_client_queue_size;
ulong proxy_server_heartbeat_period;
//#if __CONSISTEND_READ__
ulong proxy_consistend_max_delay_time;
ulong proxy_enable_consistend_read;
ulong proxy_check_consistend_period;
//#endif
ulong proxy_check_slave_lag_period;
ulong proxy_trace_max_delay_time;
ulong proxy_recv_queue_size;
ulong proxy_fail_retry_count;
my_bool proxy_format_fullpath;
my_bool proxy_format_parameterize;
my_bool proxy_not_router_mode;
my_bool proxy_digest_trace;
my_bool proxy_username_encypted;
ulong proxy_trace_busy_strategy;
ulong proxy_format_num_per_time;
ulong proxy_format_thread_num;
ulong proxy_flush_thread_num;
ulong proxy_sql_trace_thread_num;
my_bool proxy_multi_write_mode;
ulong proxy_trace_busy_wait_time_msec;
ulong proxy_trace_flush_thread_sleep_msec;
ulong proxy_trace_format_thread_sleep_msec;
ulong proxy_trace_hash_size;
ulong proxy_trace_hash_mem_length;
ulong proxy_trace_queue_array_length;
my_bool proxy_sql_trace;
ulong proxy_trace_sql_bucket_length;
char proxy_local_ip[256] = {0};
...
char *shell_username, *shell_password;
char proxy_server_uuid_space[128];
char *proxy_server_uuid = (char*)&proxy_server_uuid_space;
...
char proxy_version[SERVER_VERSION_LENGTH], *proxy_version_ptr;
...
int orig_argc;
char **orig_argv;
char* config_groups[MAX_CONFIG_GROUP];
...
void close_connection(THD *thd, uint sql_errno)
{
  ...
  if (proxy_digest_on(thd) && 
      thd->connection_type == PROXY_CONNECT_PROXY &&
      thd->trace_queue)
      thd->trace_queue->thd_state = THD_END;
...
SHOW_VAR com_status_vars[]= {
  ...
  {"config",               STMT_STATUS(SQLCOM_CONFIG)},
...
static int init_common_variables()
{
  ...
  */
  ...
  {
    if (IS_SYSVAR_AUTOSIZE(&server_version_ptr))
      sql_print_information("%s (arkproxy %s) starting as process %lu ...",
                            my_progname, server_version, (ulong) getpid());
    else
    {
      char real_server_version[SERVER_VERSION_LENGTH];
      set_server_version(real_server_version, sizeof(real_server_version));
      sql_print_information("%s (arkproxy %s as %s) starting as process %lu ...",
...
#endif
{
  ...
  */
  ...
  proxy_timeout_cache_init();
  proxy_config_thd_deinit();
  init_ssl();
  network_init();
...
static int mysql_init_variables(void)
{
  ...
  charsets_dir= 0;
  default_character_set_name= (char*) "utf8mb4";
