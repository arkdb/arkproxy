                          ...
                          const LEX_STRING *dl_arg)
{
  ...
  if (my_load_defaults(MYSQL_CONFIG_NAME, load_default_groups, &argc, &argv, NULL, NULL))
  {
                         ...
                         bool throw_error, bool locked)
{
  ...
  if (!throw_error && !var)
  {
    ...
    {
      sys_var* newvar;
      var = find_sys_var_ex(thd, "internal_null", strlen("internal_null"), 
          throw_error, locked);
      newvar = (sys_var*)my_malloc(sizeof(sys_var), MY_ZEROFILL);
      newvar->name.str = str;
      newvar->name.length = length;
      // add to last
      newvar->next = thd->null_vars;
      thd->null_vars = newvar;
      DBUG_RETURN(newvar);
    }
    my_error(ER_UNKNOWN_SYSTEM_VARIABLE, MYF(0), (int)length, (char*) str);
  }

//  if (var && !((thd->connection_type == PROXY_CONNECT_SHELL && var->hide == 0)
//      || (thd->connection_type == PROXY_CONNECT_PROXY && var->hide == 1)))
//    my_error(ER_UNKNOWN_SYSTEM_VARIABLE, MYF(0), (int)length, (char*) str);
  if (var && !(thd->connection_type == PROXY_CONNECT_PROXY && var->hide == 1)
          && thd->connection_type != PROXY_CONNECT_SHELL)
     my_error(ER_UNKNOWN_SYSTEM_VARIABLE, MYF(0), (int)length, (char*) str);
  DBUG_RETURN(var);
}
...
sys_var_pluginvar::sys_var_pluginvar(sys_var_chain *chain, const char *name_arg,
    ...
    : sys_var(chain, name_arg, pv->comment, pluginvar_sysvar_flags(pv),
              ...
              NULL, VARIABLE_NOT_IN_BINLOG, NULL, NULL, NULL, 0),
