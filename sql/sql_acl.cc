...
void update_hostname(acl_host_and_ip *host, const char *hostname)
{
...
bool compare_hostname(const acl_host_and_ip *host, const char *hostname,
			     const char *ip)
{
  long tmp;
...
*/
void login_failed_error(THD *thd)
{
                                         ...
                                         const char *data, uint data_len)
{
  ...
  *end++= protocol_version;
  ...
  end= strnmov(end, backend_server_version, SERVER_VERSION_LENGTH) + 1;
...
static ACL_USER* create_shell_user_proxy(THD* thd, MEM_ROOT *root, char* passwd)
{
    Security_context *sctx= thd->security_ctx;
    size_t scramble_length;
    void (*make_scramble)(char *, const char *, size_t);
    ...
    memset(dst, 0, sizeof(ACL_USER));
    dst->user.str= safe_strdup_root(root, sctx->user);
    dst->user.length= strlen(sctx->user);
    dst->plugin.str= native_password_plugin_name.str;
    dst->plugin.length=native_password_plugin_name.length;
    dst->auth_string.str= safe_strdup_root(root, passwd);
    dst->host.hostname= safe_strdup_root(root, sctx->priv_host);

    dst->sort= get_sort(2, dst->host.hostname, dst->user.str);
    dst->hostname_length= safe_strlen(dst->host.hostname);
    dst->user_resource.user_conn= 0;
    dst->user_resource.max_statement_time= 0.0;
    set_user_salt(dst, passwd, strlen(passwd));
    bzero(&dst->role_grants, sizeof(DYNAMIC_ARRAY));
    return dst;
}

static ACL_USER* create_shell_user(THD* thd, MEM_ROOT *root)
{
    Security_context *sctx= thd->security_ctx;
    size_t scramble_length;
    void (*make_scramble)(char *, const char *, size_t);
    ...
    if (buff == NULL)
      return 0;
    make_scramble(buff, shell_password, strlen(shell_password));

    if (strcasecmp(shell_username, sctx->user))
        return 0;
    ACL_USER *dst= (ACL_USER *) alloc_root(root, sizeof(ACL_USER));
    if (!dst)
      return 0;
    ...
    dst->user.str= safe_strdup_root(root, shell_username);
    dst->user.length= strlen(shell_username);
    dst->plugin.str= native_password_plugin_name.str;
    dst->plugin.length=native_password_plugin_name.length;
    dst->auth_string.str= safe_strdup_root(root, shell_password);
    dst->host.hostname= safe_strdup_root(root, "127.0.0.1");

    dst->sort= get_sort(2, dst->host.hostname, dst->user.str);
    dst->hostname_length= safe_strlen(dst->host.hostname);
    dst->user_resource.user_conn= 0;
    dst->user_resource.max_statement_time= 0.0;
    set_user_salt(dst, buff, strlen(buff));
    bzero(&dst->role_grants, sizeof(DYNAMIC_ARRAY));
    return dst;
}

int check_username_need_hash(char* ip, int length)
{
    if (!ip || length == 0)
        return false;

    /* if the ip in white list, then no need to hash */
    if (check_white_ip(ip, length))
        return false;

    /* check if the ip include % */
    if (get_sort(1, ip) < 128)
        return false;

    return proxy_username_encypted;
}
...
int proxy_connect_cluster(MPVIO_EXT *mpvio, char* passwd)
{
    ...
    if (proxy_user && proxy_user->conn_count >= proxy_user->max_connections)
    {
...
retry_get_auth:
    ...
    if (sctx->ip && !check_white_ip(sctx->ip, strlen(sctx->ip)))
    {
        ...
        sprintf(new_user, "%s_%s", sctx->user, sctx->ip);
        md5_16(new_user_hash, new_user);
        my_free(sctx->external_user);
        sctx->external_user= my_strdup(sctx->user, MYF(0));
    ...
    else if ((strcasecmp(sctx->host_or_ip, "localhost") == 0
             || strcasecmp(sctx->ip, "localhost") == 0
...
/**
   ...
   @retval 1    error
*/
static bool find_mpvio_user(MPVIO_EXT *mpvio, char* passwd)
{
  Security_context *sctx= mpvio->thd->security_ctx;
  ...
  else if (mpvio->thd->connection_type == PROXY_CONNECT_PROXY)
  {
      if (proxy_connect_cluster(mpvio, passwd))
          DBUG_RETURN(1);
  }
                                           ...
                                           uchar **buff, ulong pkt_len)
{
  ...
  */
  ...
  if (client_capabilities & CLIENT_PROTOCOL_41)
  {
    if (pkt_len < 32)
      return packet_error;
    // client_capabilities= uint4korr(net->read_pos);
                                        ...
                                        MYSQL_SERVER_AUTH_INFO *info)
{
  ...
  */
  ...
  if (mpvio->thd->connection_type == PROXY_CONNECT_PROXY)
      DBUG_RETURN(CR_OK);
