...
static THD* threadpool_add_connection(CONNECT *connect, void *scheduler_data)
{
  ...
  */
  ...
  /* init the thd before login */
  mysql_init_net_queue(thd);

  if ((error = create_command_executor_thread(thd)))
    goto end_free;

  /* assign trace queue 如何定义系统繁忙，一级队列满 or 二级队列满？？？*/
  if (proxy_digest_on(thd) && proxy_assign_thd_trace(thd))
      sql_print_warning("trace cache is busy, connection(%lld) can not get resource"
          ", it will retry later" ,thd->thread_id);
  ...
  */
  ...
  /* wait command_executer ready */
  while (!thd->thread_running) { my_sleep(1000); }
  ...
  /* wait command_executer ready, only when thread have running, the connection can free */
  while (!thd->thread_running) { my_sleep(1000); }
...
int proxy_decr_user_connections(THD* thd);
static void threadpool_remove_connection(THD *thd)
{
  mysql_deinit_net_queue(thd);
  proxy_decr_user_connections(thd);
  thread_attach(thd);
