...
static Sys_var_lexstring Sys_init_connect(
       "proxy_init_connect", "Command(s) that are executed for each "
       ...
       ON_CHECK(check_init_string), NULL, 0, 0);
...
static Sys_var_ulong Sys_null(
       "internal_null",
       "Timeout in seconds to wait for a lock before returning an error.",
       SESSION_VAR(internal_null), CMD_LINE(REQUIRED_ARG),
...
static Sys_var_ulong Sys_max_connections(
       "proxy_max_connections", "The number of simultaneous clients allowed",
       ...
       NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(fix_max_connections), 0, 0);

static Sys_var_ulong Sys_proxy_print_net_queue(
       "proxy_print_net_queue", "The number of simultaneous clients allowed",
       PARSED_EARLY GLOBAL_VAR(proxy_print_net_queue), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, 1),
       DEFAULT(0), BLOCK_SIZE(1), NO_MUTEX_GUARD,
       NOT_IN_BINLOG, NULL, NULL, 0, 1);
...
/* can not to use socket to login */
static Sys_var_charptr Sys_socket(
       "forbidden_proxy_socket", "Socket file to use for connection",
       READ_ONLY GLOBAL_VAR(mysqld_unix_port), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(0),
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL);
...
static Sys_var_enum Sys_thread_handling(
       ...
       READ_ONLY GLOBAL_VAR(thread_handling), CMD_LINE_HELP_ONLY,
...
static Sys_var_uint Sys_threadpool_idle_thread_timeout(
  "proxy_thread_pool_idle_timeout",
...
static Sys_var_uint Sys_threadpool_oversubscribe(
  "proxy_thread_pool_oversubscribe",
...
static Sys_var_uint Sys_threadpool_size(
 ...
 "executing threads (threads in a waiting state do not count as executing).",
  ...
  ON_UPDATE(fix_threadpool_size), 0, 0
);
static Sys_var_uint Sys_threadpool_stall_limit(
 "proxy_thread_pool_stall_limit",
 ...
 "may be created to handle remaining clients.",
  ...
  NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), 
  ON_UPDATE(fix_threadpool_stall_limit), 0, 0
);

static Sys_var_uint Sys_threadpool_max_threads(
  "proxy_thread_pool_max_threads",
  "Maximum allowed number of worker threads in the thread pool",
   GLOBAL_VAR(threadpool_max_threads), CMD_LINE(REQUIRED_ARG),
   VALID_RANGE(1, 65536), DEFAULT(65536), BLOCK_SIZE(1),
   NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), 
   ON_UPDATE(fix_tp_max_threads), 0, 0
);
...
static Sys_var_charptr Sys_general_log_path(
       "proxy_general_log_file", "Log connections and queries to given file",
       PREALLOCATED GLOBAL_VAR(opt_logname), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_log_path), ON_UPDATE(fix_general_log_file),0, 0);
...
static Sys_var_mybool Sys_general_log(
       "proxy_general_log", "Log connections and queries to a table or log file. "
       ...
       DEFAULT(FALSE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
       ON_UPDATE(fix_log_state), 0, 0);

static Sys_var_mybool Sys_reserve_client_address(
       "proxy_reserve_client_address", "reserve the client ip when beckend server is percona. ",
       GLOBAL_VAR(opt_proxy_reserve_client_address), CMD_LINE(OPT_ARG),
       DEFAULT(FALSE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
       ON_UPDATE(0), 0, 0);
...
static Sys_var_multi_source_ulonglong Sys_slave_skip_counter(
       ...
       VALID_RANGE(0, UINT_MAX), DEFAULT(0), BLOCK_SIZE(1),
       ON_UPDATE(update_slave_skip_counter), 0, 2);
...
static bool check_incoming_addresses(sys_var *self, THD *thd, set_var *var)
{
    if (check_incoming_addresses_low(var->value->name))
    {
...
static bool update_incoming_addresses(sys_var *self, THD *, enum_var_type)
{
    update_incoming_addresses_low();
}

static bool check_white_ips(sys_var *self, THD *thd, set_var *var)
{
    if (check_ip(var->value->name))
    {
        my_error(ER_IP_INCORRECT, MYF(0));
        return true;
    }
...
static bool update_white_ips(sys_var *self, THD *, enum_var_type)
{
    mysql_mutex_lock(&global_proxy_config.white_ip_lock);
    proxy_white_ips_on_update = 1;
    proxy_white_ip_t* w = LIST_GET_FIRST(global_proxy_config.white_ip_lst);
    while(LIST_GET_LEN(global_proxy_config.white_ip_lst))
    {
         proxy_white_ip_t* next = LIST_GET_NEXT(link, w);
         LIST_REMOVE(link, global_proxy_config.white_ip_lst, w);
         my_free(w);
         w = next;
    }
    
    split_white_ips();
    proxy_white_ips_on_update = 0;
    mysql_mutex_unlock(&global_proxy_config.white_ip_lock);
    return false;
}
...
static Sys_var_multi_source_ulonglong Sys_max_relay_log_size(
       ...
       ON_UPDATE(update_max_relay_log_size), 0, 2);
...
static Sys_var_charptr Sys_proxy_config_file(
       "proxy_config_file", "Server config file ",
       READ_ONLY GLOBAL_VAR(my_defaults_file),
       NO_CMD_LINE,
       IN_SYSTEM_CHARSET, DEFAULT(my_defaults_file), NO_MUTEX_GUARD, NOT_IN_BINLOG,
       NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_version(
       "proxy_version", "Server version",
       READ_ONLY GLOBAL_VAR(proxy_version_ptr), NO_CMD_LINE,
       IN_SYSTEM_CHARSET, DEFAULT(proxy_version), NO_MUTEX_GUARD, NOT_IN_BINLOG,
       NULL, NULL, 0, 0);

static const char *proxy_trace_storage_types[]=
       {"MYSQL", "KAFKA", NullS};
static Sys_var_enum Sys_proxy_trace_storage(
       "proxy_trace_storage", "binlog format to take effect over user's choice",
       GLOBAL_VAR(proxy_trace_storage), CMD_LINE(REQUIRED_ARG),
       proxy_trace_storage_types, DEFAULT(PROXY_TRACE_STORAGE_MYSQL),
        NO_MUTEX_GUARD, NOT_IN_BINLOG,
        NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_trace_storage_host(
        "proxy_trace_storage_host", "The user connect to backend",
        GLOBAL_VAR(proxy_trace_storage_host), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        NULL, NULL, 0, 0);

static Sys_var_ulong Sys_proxy_trace_storage_port(
       "proxy_trace_storage_port", "thread net message queue size, the message number",
       GLOBAL_VAR(proxy_trace_storage_port), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, 100000), DEFAULT(0), BLOCK_SIZE(1), 
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);


static Sys_var_charptr Sys_proxy_trace_storage_user(
        "proxy_trace_storage_user", "The user connect to backend",
        GLOBAL_VAR(proxy_trace_storage_user), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_trace_storage_passwd(
        "proxy_trace_storage_passwd", "The passwd connect to backend",
        GLOBAL_VAR(proxy_trace_storage_passwd), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), 
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_config_host(
        "proxy_config_host", "The user connect to backend",
        READ_ONLY GLOBAL_VAR(proxy_config_host), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_white_ips(
        "proxy_non_encrypted_ips", "proxy config backend user white ips",
        GLOBAL_VAR(proxy_white_ips), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(check_white_ips), ON_UPDATE(update_white_ips), 0, 0);

static Sys_var_ulong Sys_proxy_config_port(
       "proxy_config_port", "thread net message queue size, the message number",
       GLOBAL_VAR(proxy_config_port), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, 100000), DEFAULT(0), BLOCK_SIZE(1), 
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);


static Sys_var_charptr Sys_proxy_config_user(
        "proxy_config_user", "The user connect to backend",
        READ_ONLY GLOBAL_VAR(proxy_config_user), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_config_passwd(
        "proxy_config_passwd", "The passwd connect to backend",
        READ_ONLY GLOBAL_VAR(proxy_config_passwd), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), 
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_charptr Sys_backend_user(
        "proxy_backend_user", "The user connect to backend",
        READ_ONLY GLOBAL_VAR(backend_user), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        NULL, NULL, 0, 0);

static Sys_var_charptr Sys_namespace(
        "proxy_namespace", "The user connect to backend",
        READ_ONLY GLOBAL_VAR(proxy_namespace), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        NULL, NULL, 0, 0);

static Sys_var_charptr Sys_backend_passwd(
        "proxy_backend_passwd", "The passwd connect to backend",
        READ_ONLY GLOBAL_VAR(backend_passwd), CMD_LINE(REQUIRED_ARG),
        IN_SYSTEM_CHARSET, DEFAULT(0), 
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_ulong Sys_proxy_send_queue_size(
       "proxy_send_queue_size", "thread net message queue size, the message number",
       GLOBAL_VAR(proxy_send_queue_size), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 2000), DEFAULT(10), BLOCK_SIZE(1), 
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_trace_client_queue_size(
       "proxy_trace_client_queue_size", "thd trace node array size",
       GLOBAL_VAR(proxy_trace_client_queue_size), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(100, 100000), DEFAULT(1000), BLOCK_SIZE(1), 
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_mybool Sys_proxy_format_fullpath(
       "proxy_format_fullpath", "if format sql need fullpath",
       GLOBAL_VAR(proxy_format_fullpath), CMD_LINE(OPT_ARG), DEFAULT(FALSE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL,NULL, 0, 0);

static Sys_var_mybool Sys_proxy_format_parameterize(
       "proxy_format_parameterize", "if format sql need parameterize",
       GLOBAL_VAR(proxy_format_parameterize), CMD_LINE(OPT_ARG), DEFAULT(TRUE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL,NULL, 0, 0);

static Sys_var_mybool Sys_proxy_not_router_mode(
       "proxy_not_router_mode", "if true, proxy not route the query to read node",
       GLOBAL_VAR(proxy_not_router_mode), CMD_LINE(OPT_ARG), DEFAULT(FALSE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL,NULL, 0, 0);

static Sys_var_mybool Sys_proxy_username_encypted(
       "proxy_username_encypted", "proxy username encypted",
       GLOBAL_VAR(proxy_username_encypted), CMD_LINE(OPT_ARG), DEFAULT(TRUE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL,NULL, 0, 0);

static Sys_var_mybool Sys_proxy_digest_trace(
       "proxy_digest_trace", "proxy trace digest or not",
       GLOBAL_VAR(proxy_digest_trace), CMD_LINE(OPT_ARG), DEFAULT(FALSE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL,NULL, 0, 0);

static Sys_var_mybool Sys_proxy_sql_trace(
       "proxy_sql_trace", "proxy trace all sql or not",
       GLOBAL_VAR(proxy_sql_trace), CMD_LINE(OPT_ARG), DEFAULT(FALSE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL,NULL, 0, 0);

static Sys_var_ulong Sys_proxy_trace_busy_strategy(
       "proxy_trace_busy_strategy", "if system is busy, what is strategy we use, skip or wait.",
       GLOBAL_VAR(proxy_trace_busy_strategy), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, 5), DEFAULT(0), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_format_num_per_time(
       "proxy_format_num_per_time", "format thread loop format how many sql each time",
       GLOBAL_VAR(proxy_format_num_per_time), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 10000), DEFAULT(10), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_format_thread_num(
       "proxy_format_thread_num", "how many format thread running",
       GLOBAL_VAR(proxy_format_thread_num), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 10000), DEFAULT(5), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_flush_thread_num(
       "proxy_flush_thread_num", "how many flush thread running",
       GLOBAL_VAR(proxy_flush_thread_num), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 10000), DEFAULT(5), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_sql_trace_thread_num(
       "proxy_sql_trace_thread_num", "how many sql trace thread running",
       GLOBAL_VAR(proxy_sql_trace_thread_num), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 10000), DEFAULT(5), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_trace_busy_wait_time_msec(
       "proxy_trace_busy_wait_time", "how long wait when proxy busy",
       GLOBAL_VAR(proxy_trace_busy_wait_time_msec), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 360000000000), DEFAULT(500), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);
static Sys_var_ulong Sys_proxy_trace_flush_thread_sleep_msec(
       "proxy_trace_flush_thread_sleep_msec", "how long flush tread sleep",
       GLOBAL_VAR(proxy_trace_flush_thread_sleep_msec), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 360000000000), DEFAULT(500), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);
static Sys_var_ulong Sys_proxy_trace_format_thread_sleep_msec(
       "proxy_trace_format_thread_sleep_msec", "how long format thread sleep",
       GLOBAL_VAR(proxy_trace_format_thread_sleep_msec), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 360000000000), DEFAULT(500), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);
static Sys_var_ulong Sys_proxy_trace_hash_size(
       "proxy_trace_hash_size", "how many node in hash per thread",
       GLOBAL_VAR(proxy_trace_hash_size), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 100000000), DEFAULT(200), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);
static Sys_var_ulong Sys_proxy_trace_hash_mem_length(
       "proxy_trace_hash_mem_length", "Max memery hash can use per thread,default 128M",
       GLOBAL_VAR(proxy_trace_hash_mem_length), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 549755813888), DEFAULT(134217728), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);
static Sys_var_ulong Sys_proxy_trace_queue_array_length(
       "proxy_trace_queue_array_length", "how many trace queue",
       GLOBAL_VAR(proxy_trace_queue_array_length), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 100000000), DEFAULT(500), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_trace_sql_bucket_length(
       "proxy_trace_sql_bucket_length", "how long one sql bucket for format node",
       GLOBAL_VAR(proxy_trace_sql_bucket_length), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 100000000), DEFAULT(100), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_server_heartbeat_period(
       "proxy_server_heartbeat_period", "heartbeat between proxy and servers",
       GLOBAL_VAR(proxy_server_heartbeat_period), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(10, 100000), DEFAULT(100), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

#if __CONSISTEND_READ__
static Sys_var_ulong Sys_proxy_check_consistend_period(
       "proxy_check_consistend_period", "heartbeat between proxy and servers",
       GLOBAL_VAR(proxy_check_consistend_period), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 100000), DEFAULT(10), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_consistend_max_delay_time(
       "proxy_consistend_max_delay_time", "heartbeat between proxy and servers",
       GLOBAL_VAR(proxy_consistend_max_delay_time), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 1000000), DEFAULT(1000), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_enable_consistend_read(
       "proxy_enable_consistend_read", "The number of simultaneous clients allowed",
       GLOBAL_VAR(proxy_enable_consistend_read), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, 1),
       DEFAULT(0), BLOCK_SIZE(1), NO_MUTEX_GUARD,
       NOT_IN_BINLOG, NULL, NULL, 0, 0);
#endif

static Sys_var_ulong Sys_proxy_check_slave_lag_period(
       "proxy_check_slave_lag_period", "heartbeat between proxy and servers",
       GLOBAL_VAR(proxy_check_slave_lag_period), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 100000), DEFAULT(10), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_trace_max_delay_time(
       "proxy_trace_max_delay_time", "trace info max delay time",
       GLOBAL_VAR(proxy_trace_max_delay_time), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 100000), DEFAULT(3), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_recv_queue_size(
       "proxy_recv_queue_size", "thread net message queue size, the message number",
       GLOBAL_VAR(proxy_recv_queue_size), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(100, 10000), DEFAULT(1000), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_ulong Sys_proxy_fail_retry_count(
       "proxy_fail_retry_count", "retry count when execution fail",
       GLOBAL_VAR(proxy_fail_retry_count), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, 100), DEFAULT(3), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 1);

static Sys_var_mybool Sys_proxy_multi_write_mode(
       "proxy_multi_write_mode", "if enabled, the all node can been write"
       " used in switchover and failover, weather the conns must been killed",
       GLOBAL_VAR(proxy_multi_write_mode), CMD_LINE(OPT_ARG), DEFAULT(FALSE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL,NULL, 0, 0);
...
static Sys_var_double Sys_proxy_net_buffer_outlier_scale(
       ...
       GLOBAL_VAR(proxy_net_buffer_outlier_scale), CMD_LINE(OPT_ARG),
       VALID_RANGE(0, 100), DEFAULT(1.5), NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_ulong Sys_proxy_net_per_buffer_max_size(
       "proxy_net_per_buffer_max_size", "max buffer size for per connection buffer",
       GLOBAL_VAR(proxy_net_per_buffer_max_size), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, ULONG_MAX), DEFAULT(0), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);
...
static Sys_var_uint Sys_shell_listen_port(
       "proxy_shell_listen_port",
       "shell port number to use for tcp connections in a proxy manegment ",
       READ_ONLY GLOBAL_VAR(mysqld_extra_port), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, UINT_MAX32), DEFAULT(3439), BLOCK_SIZE(1),
       NULL, NOT_IN_BINLOG,NULL, NULL,0, 0);

static Sys_var_charptr Sys_shell_username(
       "proxy_shell_username", "user name for login proxy shell",
       READ_ONLY GLOBAL_VAR(shell_username), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(0),
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_server_uuid(
       "proxy_server_uuid", "uuid",
       READ_ONLY GLOBAL_VAR(proxy_server_uuid), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(proxy_server_uuid),
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_charptr Sys_shell_password(
       "proxy_shell_password", "password for login proxy shell",
       READ_ONLY GLOBAL_VAR(shell_password), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(0),
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_server_address(
       "proxy_server_address", "the proxy server ips, for proxy transfer and change",
       READ_ONLY GLOBAL_VAR(proxy_server_address), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(proxy_local_ip),
        NO_MUTEX_GUARD, NOT_IN_BINLOG, NULL, NULL, 0, 0);

static Sys_var_charptr Sys_proxy_incoming_addresses(
       "proxy_incoming_addresses", "the proxy server ips, proxy cluster",
       GLOBAL_VAR(proxy_incoming_addresses), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(""),
        NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_incoming_addresses), 
        ON_UPDATE(update_incoming_addresses), 0, 0);
...
static Sys_var_ulong Sys_extra_max_connections(
       ...
       VALID_RANGE(1, 100000), DEFAULT(10), BLOCK_SIZE(1), NO_MUTEX_GUARD,
