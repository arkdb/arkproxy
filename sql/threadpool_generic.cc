...
void mysqld_list_thread_pool(THD *thd,const char *user, bool verbose)
{
    THD* tmp;
    Item *field;
    ...
    field_list.push_back(field= new (mem_root)
                         Item_return_int(thd, "Thread_Count", 7, MYSQL_TYPE_LONG),
                         mem_root);
    field_list.push_back(field= new (mem_root)
                         Item_return_int(thd, "Active_Thread_Count", 7, MYSQL_TYPE_LONG),
                         mem_root);
    field_list.push_back(field= new (mem_root)
                         Item_return_int(thd, "Connection_Count", 7, MYSQL_TYPE_LONG),
                         mem_root);
    field_list.push_back(field= new (mem_root)
                         Item_return_int(thd, "Queue_Event_Count", 7, MYSQL_TYPE_LONG),
                         mem_root);
    field_list.push_back(field= new (mem_root)
                         Item_return_int(thd, "Stalled", 7, MYSQL_TYPE_LONG),
                         mem_root);
    ...
    {
        if (all_groups[i].pollfd < 0)
            break;
        protocol->prepare_for_resend();
        protocol->store((ulonglong)i);
        protocol->store(all_groups[i].thread_count);
        protocol->store(all_groups[i].active_thread_count);
        protocol->store(all_groups[i].connection_count);
        protocol->store(all_groups[i].queue_event_count);
        protocol->store(all_groups[i].stalled);
