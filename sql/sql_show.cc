...
bool write_out_var_exceptions(const char* name,str_t* outstr)
{
    ...
    {
        sprintf(vname, "%-40s", name);
        str_append(outstr, vname);
        str_append(outstr, "\n");
    }
    return false;
}

bool write_status_array_for_proxy(
    THD *thd, 
    SHOW_VAR *variables,
    str_t* outstr
)
{
    ...
    DBUG_ENTER("write_status_array_for_proxy");
        ...
        {
          sys_var *var= ((sys_var *) value);
          if (var->hide || var->option.id == sys_var::NO_GETOPT)
          {
              mysql_mutex_unlock(&LOCK_global_system_variables);
              continue;
          }
          show_type= var->show_type();
          value= (char*) var->value_ptr(thd, OPT_GLOBAL, &null_lex_str);
          charset= var->charset(thd);
        }
        ...
        sprintf(vname, "%-40s", variables->name);
        str_append(outstr, vname);
        str_append(outstr, " = ");
        str_append(outstr, pos);
        str_append(outstr, "\n");
...
static bool show_status_array_for_proxy(THD *thd, const char *wild,
    SHOW_VAR *variables,
    ...
    int showall)
{
    ...
    MEM_ROOT *mem_root= thd->mem_root;
    List<Item>    field_list;
    DBUG_ENTER("show_status_array_for_proxy");
    ...
    Item *field;
    field_list.push_back(field=new (mem_root)
                         Item_empty_string(thd, "Variable_name", NAME_CHAR_LEN),
                         mem_root);
    field_list.push_back(field=new (mem_root)
                         Item_empty_string(thd, "Value", NAME_CHAR_LEN),
                         mem_root);
          ...
          Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
    {
            ...
            wild_case_compare(system_charset_info, name_buffer, wild)))
      {
        ...
        protocol->store(name_buffer, thd->charset());
        // if (strlen(pos) && (!strcasecmp("inception_password", name_buffer) ||
        //     !strcasecmp("inception_remote_system_password", name_buffer)))
        // {
        //   char m_hashed_password_buffer[CRYPT_MAX_PASSWORD_SIZE + 1];
        //   String str(pos, system_charset_info);
        //   calculate_password(&str, m_hashed_password_buffer);
        //   protocol->store(m_hashed_password_buffer, thd->charset());
        // }
        // else 
          protocol->store(pos, thd->charset());
...
{
  DBUG_ENTER("mysql_local_show_variables");
  int res= 0;
  LEX *lex= thd->lex;
  const char *wild= lex->wild ? lex->wild->ptr() : NullS;
  bool sorted_vars=true;

  enum_var_type option_type= OPT_GLOBAL;
  bool upper_case_names = 0;
  mysql_rwlock_rdlock(&LOCK_system_variables_hash);
  if(thd->lex->ident.length==0)
      res= show_status_array_for_proxy(thd, wild, 
          enumerate_sys_vars(thd, sorted_vars, option_type),
