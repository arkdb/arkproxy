...
void LOGGER::init_log_tables()
{
  ...
  if (!is_log_tables_initialized && !file_log_handler->init())
...
bool LOGGER::activate_log_handler(THD* thd, uint log_type)
{
  ...
  case QUERY_LOG_SLOW:
    if (!global_system_variables.sql_log_slow)
    {
      ...
      if (table_log_handler && table_log_handler->activate_log(thd, QUERY_LOG_SLOW))
      {
  ...
  case QUERY_LOG_GENERAL:
    if (!opt_log)
    {
      ...
      if (table_log_handler && table_log_handler->activate_log(thd, QUERY_LOG_GENERAL))
      {
        /* Error printed by open table in activate_log() */
        res= TRUE;
        file_log->close(0);
