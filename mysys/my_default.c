...
static int handle_default_option(void *, const char *, const char *, char** groups_name);
...
static int search_default_file(Process_option_func func, void *func_ctx,
			       const char *dir, const char *config_file, char** groups_name);
  ...
  */
  ...
  if (my_defaults_file)
  {
    if ((error= search_default_file_with_ext(func, func_ctx, "", "",
                                             my_defaults_file, 0, groups_name)) < 0)
      goto err;
    if (error > 0)
    {
  ...
  {
    if ((error= search_default_file(func, func_ctx, NullS, conf_file, groups_name)) < 0)
      goto err;
  }
  else
  {
    for (dirs= default_directories ; *dirs; dirs++)
    {
      if (**dirs)
      {
	if (search_default_file(func, func_ctx, *dirs, conf_file, groups_name) < 0)
	  goto err;
      }
      else if (my_defaults_extra_file)
      {
        if ((error= search_default_file_with_ext(func, func_ctx, "", "",
                                                my_defaults_extra_file, 0, groups_name)) < 0)
...
static int handle_default_option(void *in_ctx, const char *group_name,
                                 const char *option, char** groups_name)
{
  char *tmp;
  ...
  if (!option)
  {
    ...
    {
        int i = 0;
        const char** groups = ctx->group->type_names;
        for (i = 0; i < MAX_CONFIG_GROUP; i++)
        {
            if (groups_name[i] && !strcasecmp(groups_name[i], group_name))
                break;
            if (!groups_name[i])
                break;
        }
        ...
        for (; *groups ; groups++)
        {
            if (!strcasecmp(*groups, group_name))
                return 0;
        }

        if (!groups_name[i])
        {
            char* name = my_malloc(strlen(group_name)+1, MY_ZEROFILL);
            strcpy(name, group_name);
            groups_name[i] = name;
            if (i >= MAX_CONFIG_GROUP)
            {
                fprintf(stderr,"configure file invalid, too many config groups\n");
                return 1;
            }
            groups_name[i+1] = NULL;
        }
        else
        {
            sprintf(tmp_chr, "Config file invalid, "
                    "duplicate config groups: '%s'\n", group_name);
            fprintf(stderr, "%s", tmp_chr);
            return 1;
        }
    }
    return 0;
...
int load_defaults(const char *conf_file, const char **groups,
                  int *argc, char ***argv, char** groups_name)
{
  return my_load_defaults(conf_file, groups, argc, argv, &default_directories, groups_name);
}
...
int my_load_defaults(const char *conf_file, const char **groups,
                  int *argc, char ***argv, const char ***default_directories, char** groups_name)
{
  DYNAMIC_ARRAY args;
  TYPELIB group;
                   ...
                   char** groups_name)
{
  ...
  for (ext= (char**) exts_to_use; *ext; ext++)
  {
    ...
    if ((error= search_default_file_with_ext(opt_handler, handler_ctx,
                                             dir, *ext, config_file, 0, groups_name)) < 0)
      return error;
  }
  return 0;
}
                                        ...
                                        char** groups_name)
{
  ...
  while (mysql_file_fgets(buff, sizeof(buff) - 1, fp))
  {
	...
	goto err;
      ...
      if (opt_handler(handler_ctx, curr_gr, NULL, groups_name))
          goto err;
    ...
    if (!value)
    {
      ...
      if (opt_handler(handler_ctx, curr_gr, option, NULL))
        goto err;
    }
    else
    {
